...
content = None
with open("with_normal.txt", "r") as file:
    content = file.read()

content = content.split("\n")
point_1 = [float(content[i]) for i in range(1,len(content),6)]
point_2 = [float(content[i]) for i in range(2,len(content),6)] # error bar
point_3 = [float(content[i]) for i in range(3,len(content),6)]
point_4 = [float(content[i]) for i in range(4,len(content),6)] # error bar
...
plt.xlabel("k values")
plt.ylabel("accuracy")
plt.title("test data evaluated by training data, with normalization")
...
plt.xlabel("k values")
plt.ylabel("accuracy")
plt.title("training data evaluated by training data, with normalization")
