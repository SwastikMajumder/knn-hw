...
for k in range(1,51+1,2): # k value
    ...
    for _ in range(20):
        ...
        for i in range(len(test_f)):    
            output = classify((train_f-min_c)/(max_c-min_c), (test_f[i]-min_c)/(max_c-min_c), k, train_c)
        ...
        for i in range(len(train_f)):    
            output = classify((train_f-min_c)/(max_c-min_c), (train_f[i]-min_c)/(max_c-min_c), k, train_c)
            if output == train_c[i]:
                correct += 1
